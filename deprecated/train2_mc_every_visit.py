...
try:
    ...
    from agents.monte_carlo_agent_every_visit import MonteCarloAgent_EV
...
def parse_args():
    ...
    p.add_argument("--epsilon_min",  type=float, default=0.2,
...
def main(GRID, no_gui, sigma, fps, episodes, random_seed,
    ...
    for grid in GRID:
        ...
        agent = MonteCarloAgent_EV(n_actions=4,
                                epsilon=epsilon,
                                gamma=gamma)
        ...
        # Decay rate for epsilon decay
        decay_rate = -np.log((epsilon - epsilon_min) / (epsilon - epsilon_min)) / (episodes)
        ...
        for ep in trange(episodes, desc="Episodes"):
            ...
            # Epsilon decay from epsilon (1) to epsilon_min(0.2 by default)
            ...
            # int(grid_size * (1 + np.log1p(ep*0.1))) #needs work
